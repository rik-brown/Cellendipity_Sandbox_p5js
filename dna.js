...
function DNA(newgenes) {
  if (newgenes) {           // Tests to see if the function is called with a newgenes passed in or not:
    ...
    this.genes[3] = 25/255; // Override
    this.genes[7] = 5/255;  // Override
    if (random(1)>0.5) {this.genes[11] = 1/3;} else {this.genes[11] = random(0, 1);} // 11 = flatness
    if (random(1)>0.6) {this.genes[12] = 0.5;} else {this.genes[12] = random(0, 1);} // 12 = spiral screw
    this.genes[13] = 1; // Override = 75
    this.genes[14] = 0.6; // Override = 3
