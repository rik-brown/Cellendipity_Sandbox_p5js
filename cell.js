...
function Cell(vel, dna) {
  ...
  this.lifespan = map(this.dna.genes[10], 0, 1, width*0.3,  width*0.4); // Lifespan can be lowered by DNA but not increased
  this.fertility = map(this.dna.genes[13], 0, 1, 0.5, 0.75); // How soon will the cell become fertile?
  this.spawnCount = int(map(this.dna.genes[14], 0, 1, 1, 5)); // Max. number of spawns

  // SIZE AND SHAPE
  this.cellStartSize = map(this.dna.genes[8], 0, 1, width*0.05, width*0.12);
  this.cellEndSize = this.cellStartSize * map(this.dna.genes[9], 0, 1, 0.05, 0.3);
  //this.r = this.cellStartSize; // Initial value for radius
  this.flatness = map(this.dna.genes[11], 0, 1, 0.75, 1.5); // To make circles into ellipses. range 0.5 - 1.5
  ...
  this.noisePercent = this.dna.genes[17]; // How much influence on velocity does Perlin noise have?
  this.spiral = map(this.dna.genes[12], 0, 1, -0.3, 0.3); // Spiral screw amount
  this.vMax = map(this.dna.genes[15], 0, 1, 0, 4); // Maximum magnitude in velocity components generated by noise
  this.xoff = random(1000); //Seed for noise
  this.yoff = random(1000); //Seed for noise
  this.step = map(this.dna.genes[16], 0, 1, 0.001, 0.006); //Step-size for noise

  // COLOUR

  // FILL COLOR
  this.fill_H = map(this.dna.genes[0], 0, 1, 0, 360);
  this.fill_S = map(this.dna.genes[1], 0, 1, 128, 255);
  this.fill_B = map(this.dna.genes[2], 0, 1, 196, 255);
